...
class PREDICTOR{
 ...
 private:
  ...
  UINT32  *pht_1;        // pattern history table
  UINT32  *pht_2;        // pattern history table
  UINT32  historyLength_1; // history length
  UINT32  historyLength_2; // history length
  UINT32  numPht_1_Entries; // entries in pht
  UINT32  numPht_2_Entries; // entries in pht
...
PREDICTOR::PREDICTOR(void){
  ...
  historyLength_1    = HIST_LEN_1;
  historyLength_2    = HIST_LEN_2;
  ghr                = 0;
  numPht_1_Entries   = (1<< HIST_LEN_1);
  numPht_2_Entries   = (1<< HIST_LEN_1) * (1<< HIST_LEN_2);

  pht_1 = new UINT32[numPht_1_Entries];
  pht_2 = new UINT32[numPht_2_Entries];

  for(UINT32 ii=0; ii< numPht_1_Entries; ii++){
    pht_1[ii]=0; 
  }
  for(UINT32 ii=0; ii< numPht_2_Entries; ii++){
    pht_2[ii]=PHT_CTR_INIT; 
  }
...
bool   PREDICTOR::GetPrediction(UINT64 PC){
  ...
  UINT32 pht_1_Index   = (ghr) % (numPht_1_Entries);
  UINT32 pht_2_Index   = (pht_1_Index << historyLength_2) + ((pht_1[pht_1_Index]) % (1<< HIST_LEN_2));
  UINT32 phtCounter = pht_2[pht_2_Index];
...
void  PREDICTOR::UpdatePredictor(UINT64 PC, OpType opType, bool resolveDir, bool predDir, UINT64 branchTarget){
  ...
  if(resolveDir == TAKEN){
    pht_2[pht_2_Index] = SatIncrement(phtCounter, PHT_CTR_MAX);
  }else{
    pht_2[pht_2_Index] = SatDecrement(phtCounter);
  }

  // update the PHT_1
  
  pht_1[pht_1_Index] = (pht_1[pht_1_Index] << 1);
