...
//  printf(" ghr: %x index: %x counter: %d prediction: %d\n", ghr, phtIndex, phtCounter, phtCounter > PHT_CTR_MAX/2);
  ...
  if(meta_predictor[btbIndex] > (PHT_CTR_MAX/2)){
	  ...
	  if(bimodal[btbIndex] > (PHT_CTR_MAX/2)){return TAKEN;
	  }
	  else{return NOT_TAKEN;
	  }
  }
...
void  PREDICTOR::UpdatePredictor(UINT64 PC, OpType opType, bool resolveDir, bool predDir, UINT64 branchTarget){
  ...
  if(((phtCounter/2) >= 1) == resolveDir){
	  if(((bimodal[btbIndex]/2) >= 1) != resolveDir){
		  meta_predictor[btbIndex] = SatIncrement(meta_predictor[btbIndex], PHT_CTR_MAX);
	  }
  }
  if(((phtCounter/2) >= 1) != resolveDir){
	  if(((bimodal[btbIndex]/2) == resolveDir){
  ...
  if(resolveDir == TAKEN){
	 ...
	 bimodal[btbIndex] = SatIncrement(bimodal[btbIndex], PHT_CTR_MAX);
  }
  else{
    pht[phtIndex] = SatDecrement(phtCounter);
	bimodal[btbIndex] = SatDecrement(bimodal[btbIndex]);
  }
