...
#define META_LEN (VECTOR_LEN - 0)

#define HIST_LEN  15
#define SHFT_AMOUNT 2

//stats
  UINT64  NumMispred = 0, NumBranches = 0, pht1_Absolute_Mispred = 0, Numpht1 = 0, Numpht2 = 0, Numbimodal = 0, pht1_Relative_Mispred = 0, pht2_Absolute_Mispred = 0,
		  pht2_Relative_Mispred = 0, bimodal_Absolute_Mispred = 0, bimodal_Relative_Mispred, metaChoosedGskew = 0, metaChoosedBimodal = 0, metaChoosedWrong = 0;
//stats
...
PREDICTOR::PREDICTOR(void){
  ...
  meta = new UINT32[numMetaEntries];

  for(UINT32 ii=0; ii< numMetaEntries; ii++){
    meta[ii]= 0;									//0 so that bimodal is prefered on the begining ---> better warm up performance
  }
...
bool   PREDICTOR::GetPrediction(UINT64 PC){
  ...
  UINT32 meta_index = (PC >> SHFT_AMOUNT) % numMetaEntries;
  ...
  UINT32 meta_Counter = meta[meta_index];
  
	if(meta_Counter < PHT_CTR_MAX/2){
...
//Update
	...
	if((bimodal_Counter > PHT_CTR_MAX/2) != egskewOutcome){				//Update META predictor if predictions differ
	 
		if(egskewOutcome == resolveDir){
		 
			SatIncrement(meta[meta_index], PHT_CTR_MAX);
		
		}
		else{
		  
			SatDecrement(meta[meta_index], PHT_CTR_MAX);
		}
	}
