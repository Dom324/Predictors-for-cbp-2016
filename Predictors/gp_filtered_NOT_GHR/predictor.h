...
#define NUM_BTB_ENTRIES 2048
#define DIR_CNT_MAX 24
#define HIST_LEN   17
...
PREDICTOR::PREDICTOR(void){
  ...
  DirCount = new UINT32[NUM_BTB_ENTRIES];
  ...
  DirBit = new UINT32[NUM_BTB_ENTRIES];
...
//  printf(" ghr: %x index: %x counter: %d prediction: %d\n", ghr, phtIndex, phtCounter, phtCounter > PHT_CTR_MAX/2);
  ...
  if(DirCount[BTBIndex] == DIR_CNT_MAX){
	  if(DirBit[BTBIndex] == 1){ 
        return TAKEN; 
      }
      else{
        return NOT_TAKEN; 
...
void  PREDICTOR::UpdatePredictor(UINT64 PC, OpType opType, bool resolveDir, bool predDir, UINT64 branchTarget){
  ...
  if(resolveDir == DirBit[BTBIndex]){
	  if(DirCount[BTBIndex] != DIR_CNT_MAX){
		  DirCount[BTBIndex] = DirCount[BTBIndex] + 1;
	  }
  }
  else{
	  DirBit[BTBIndex] = resolveDir;
	  DirCount[BTBIndex] = 0;
  }
	  

  
  if((resolveDir != DirBit[BTBIndex]) || (DirCount[BTBIndex] != DIR_CNT_MAX)){
