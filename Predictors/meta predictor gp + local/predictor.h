...
#define NUM_BTB_ENTRIES 4096
#define HIST_LEN_LOCAL 16
#define HIST_LEN_GP   16
...
class PREDICTOR{
 ...
 private:
  UINT32  *meta_predictor;
  UINT32  *lhr;
  UINT32  *pht_local;
  UINT32  ghr;           // global history register
  UINT32  *pht_gp;          // pattern history table
  UINT32  historyLength; // history length
  UINT32  numPhtEntries; // entries in pht 
  UINT32  numPht_local_Entries;
...
PREDICTOR::PREDICTOR(void){
  ...
  historyLength    = HIST_LEN_GP;
  ghr              = 0;
  numPhtEntries    = (1<< HIST_LEN_GP);
  numPht_local_Entries = (1<< HIST_LEN_LOCAL);

  pht_gp = new UINT32[numPhtEntries];

  for(UINT32 ii=0; ii< numPhtEntries; ii++){
    pht_gp[ii]=PHT_CTR_INIT; 
  }
  
  lhr = new UINT32[NUM_BTB_ENTRIES];
  
  for(UINT32 ii=0; ii< NUM_BTB_ENTRIES; ii++){
    lhr[ii]=0; 
  }
  
  pht_local = new UINT32[numPht_local_Entries];

  for(UINT32 ii=0; ii< numPht_local_Entries; ii++){
    pht_local[ii]=PHT_CTR_INIT; 
  }
  
  meta_predictor = new UINT32[NUM_BTB_ENTRIES];
  
  for(UINT32 ii=0; ii< NUM_BTB_ENTRIES; ii++){
    meta_predictor[ii]=PHT_CTR_INIT; 
  }
...
bool   PREDICTOR::GetPrediction(UINT64 PC){
  ...
  UINT32 pht_local_Index   = (lhr[btbIndex]) % (numPhtEntries);
  UINT32 pht_gp_Index   = (PC^ghr) % (numPhtEntries);
  UINT32 phtCounter = pht_gp[pht_gp_Index];

//  printf(" ghr: %x index: %x counter: %d prediction: %d\n", ghr, phtIndex, phtCounter, phtCounter > PHT_CTR_MAX/2);

  
  if(meta_predictor[btbIndex] > (PHT_CTR_MAX/2)){
     ...
     if(pht_local[pht_local_Index] > (PHT_CTR_MAX/2)){
        return TAKEN;
	 }
	 else{
		 return NOT_TAKEN;
...
void  PREDICTOR::UpdatePredictor(UINT64 PC, OpType opType, bool resolveDir, bool predDir, UINT64 branchTarget){
  ...
  UINT32 btbIndex   = (PC) % (NUM_BTB_ENTRIES);
  UINT32 pht_local_Index   = (lhr[btbIndex]) % (numPht_local_Entries);
  UINT32 pht_gp_Index   = (PC^ghr) % (numPhtEntries);
  UINT32 phtCounter = pht_gp[pht_gp_Index];
  
  
  if((phtCounter > (PHT_CTR_MAX/2)) == resolveDir){
	  if((pht_local[pht_local_Index] > (PHT_CTR_MAX/2)) != resolveDir){
		  meta_predictor[btbIndex] = SatIncrement(meta_predictor[btbIndex], PHT_CTR_MAX);
	  }
  }
  if((phtCounter > (PHT_CTR_MAX/2)) != resolveDir){
	  if((pht_local[pht_local_Index] > (PHT_CTR_MAX/2)) == resolveDir){
		  meta_predictor[btbIndex] = SatDecrement(meta_predictor[btbIndex]);
	  }
  }
  

  if(resolveDir == TAKEN){
    pht_gp[pht_gp_Index] = SatIncrement(phtCounter, PHT_CTR_MAX);
	pht_local[pht_local_Index] = SatIncrement(pht_local[pht_local_Index], PHT_CTR_MAX);
  }else{
    pht_gp[pht_gp_Index] = SatDecrement(phtCounter);
	pht_local[pht_local_Index] = SatDecrement(pht_local[pht_local_Index]);
  }
