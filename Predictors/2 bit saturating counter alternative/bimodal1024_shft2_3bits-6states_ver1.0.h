...
void  PREDICTOR::UpdatePredictor(UINT64 PC, OpType opType, bool resolveDir, bool predDir, UINT64 branchTarget){
  ...
  if(resolveDir == TAKEN){
	...
	if(bimodalCounter != 1 && bimodalCounter != PHT_CTR_MAX && bimodalCounter != 2){
		
		bimodal[BimodalIndex]++;
		
	}
	else if(bimodalCounter == 1){
		
		bimodal[BimodalIndex] = 3;
		
	}
	else if(bimodalCounter == 2){
		
		bimodal[BimodalIndex] = 1;
	...
	if(bimodalCounter != 4 && bimodalCounter != 0 && bimodalCounter !=  3){
		...
		bimodal[BimodalIndex] = 2;
		
	}
	else if(bimodalCounter == 3){
		
		bimodal[BimodalIndex] = 4;
